import { CHON_GHE, HUY_GHE, DAT_GHE } from "../actions/types/BTBookTicketsType";
...
const BTBookTicketsReducer = (state = stateDefault, action) => {
  switch (action.type) {
    case CHON_GHE: {
      if (action.dsGhe.daDat) {
        return { ...state };
      }
      let choseChairUpdate = [...state.choseChair];
      let key = false;
      choseChairUpdate.map((ghe, index) => {
        //Nếu ghế đã được chọn thì xóa khỏi state
        if (ghe.soGhe === action.dsGhe.soGhe) {
          choseChairUpdate.splice(index, 1);
          //tính tổng tiền
          state.total -= action.dsGhe.gia;
          return (key = true);
        }
        return key;
      });
      //Nếu ghế chưa được chọn thì gán vào state
      if (!key) {
        choseChairUpdate.push(action.dsGhe);
        //tính tổng tiền
        state.total += action.dsGhe.gia;
      }
      state.choseChair = choseChairUpdate;
      return { ...state };
    }
    ...
    case HUY_GHE: {
      ...
      choseChairUpdate.map((veGhe, index) => {
        if (veGhe.soGhe === action.ghe.soGhe) {
          choseChairUpdate.splice(index, 1);
        }

        return choseChairUpdate;
      });
      //tính tổng tiền
      state.total -= action.ghe.gia;
    ...
    case DAT_GHE: {
      let chairUpdate = [...state.lineChair];
      for (let key in chairUpdate) {
        chairUpdate[key].danhSachGhe.map((ghe, index) => {
          for (let sp of state.choseChair) {
            if (sp.soGhe === ghe.soGhe) {
              sp.daDat = true;
            }
          }
          return chairUpdate;
...
export default BTBookTicketsReducer;
